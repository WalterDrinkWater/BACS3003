...
def Get_Programme_Details(progID):
    if request.method == "GET":
        ...
        cursor.close()
        return render_template(
            "ProgDetails.html", prog=prog, course=course, req=grouped_data, ov=ov
        )
...
def Compare_Programme(progID):
    ...
    if request.method == "GET":
        ...
        progListSql = (
            "Select programmeName,programmeID from Programme WHERE programmeID != %s"
        )
        ...
        for prog_name, alevel_sub, alevel_grade, diploma_grade in allReq:
            prog_data.append(
                {"Program": prog_name, "Subject": alevel_sub, "Grade": alevel_grade}
            )
        print(allReq)
    cursor.close()
    return render_template(
